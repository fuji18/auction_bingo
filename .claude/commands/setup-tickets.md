...
チケットを git の外(Issues)に置くことで、ステータス更新のためのコミットが不要になり、複数ブランチ・エージェントの並行作業でも競合しません。クローズは `develop` マージ時に手動で行います(feature PR は `develop` 宛てで `Closes #N` が自動発火しないため。`/next-ticket` ステップ4 / CLAUDE.md「チケット運用」節)。
...
develop マージ時に手動クローズします（/next-ticket ステップ4）。
```
