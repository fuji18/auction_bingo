...
| 役割 | モデル | 設定箇所 |
|---|---|---|
| **司令塔(メインセッション)**: 計画・設計判断・統合・承認・synthesis | **Opus** (`claude-opus-4-8`) | `.claude/settings.json` の `"model": "opus"` |
...
|---|---|
| CLAUDE.md の新規生成 | **既存 CLAUDE.md に「ハーネス」節を追記**(全体で 150 行以内を維持。超える場合は詳細をスキルへ退避) |
...
- `state.json`: `{"version": 2, "model": "claude-opus-4-8", "agent_teams_enabled": {Q5}, "teammate_mode": "{Q6}", "initialized_at": "{ISO8601}"}`
