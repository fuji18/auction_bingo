...
- **実装を始める前に `develop` が `main` に追いついているか確認する**(遅れていれば同期してから feature を切る)
...
6. **PR を作成する**: ベースは `develop`。`Closes #N` を含める(develop 宛てでは自動発火しないが、将来 main へ直接入った場合の保険)。未完了項目があれば PR ボディに明記する
7. **記録する**: Issue に `.steering/` のディレクトリ名と PR URL をコメントする
8. **クローズする(手動)**: PR が `develop` へマージされたら `gh issue close #N` で手動クローズし、`in-progress` ラベルを外す。**`develop` 宛て PR では `Closes #N` が自動発火しない**(発火はデフォルトブランチ = `main` へのマージ時のみ)ため、develop マージ時の手動クローズで統一する。これにより後続チケットの `depends: #N`(参照先が closed か)が正しく解決される
